// Jaspal Universal Ollama Live Chat Widget - Fixed UTF8 - No Emoji Corruption
(function(){
  const cfg = document.currentScript?.dataset || {};
  const PHONE = cfg.phone || '919958037734';
  const MODEL = cfg.model || 'llama3.2';
  const BRIDGE = cfg.bridge || 'http://localhost:3001/api/chat';
  const DIRECT = cfg.direct || 'http://localhost:11434/api/chat';

  const css = `
  #jaspal-chat-btn{position:fixed;bottom:20px;right:20px;width:62px;height:62px;border-radius:50%;background:linear-gradient(135deg,#1e40af,#3b82f6);color:#fff;border:none;box-shadow:0 8px 24px rgba(30,64,175,.4);cursor:pointer;z-index:999999;display:flex;align-items:center;justify-content:center;font-size:24px;font-weight:800;transition:transform .2s}
  ...
  const btn = document.createElement('button'); btn.id='jaspal-chat-btn'; btn.innerHTML='AI<span class="ai-badge">LIVE</span>';
  const win = document.createElement('div'); win.id='jaspal-chat-win';
  win.innerHTML=`
    <div class="jch-head"><div><b>Jaspal AI Assistant</b><br><small><span style="display:inline-block;width:8px;height:8px;background:#22c55e;border-radius:50%;margin-right:4px"></span>Live - AI + Human - 2 min reply</small></div><button id="jch-close" style="background:rgba(255,255,255,.2);border:none;color:#fff;width:28px;height:28px;border-radius:50%;cursor:pointer">X</button></div>
    <div class="jch-body" id="jch-body">
      <div class="jch-msg bot">Namaste! Aur kya chahiye? Car DL8C Rs12450, Health Floater Rs18200, Life 1Cr, Mutual Fund Nifty +14.2% Rs2.4L, IPOs? Aapke liye koi naya plan banane ki zaroorat hai? WhatsApp 9958037734 par 5 min quote kholne ke liye kya jaldi karenge?</div>
  ...
  let history = [{role:'system', content: 'You are Jaspal Singh Quickinsure Partner Connaught Place Delhi. Help with Car DL8C Rs12450, Health Floater Rs18200, Life 1Cr, Mutual Fund Nifty +14.2% Rs2.4L, IPOs. Hinglish friendly, concise, helpful. Always mention WhatsApp 9958037734 for 5 min quote.'}];
  ...
  function addMsg(text, who){
    ...
    try{
      const payload={page:location.href, text, who, time:new Date().toISOString()};
  ...
  async function askOllama(userText){
    ...
    for(let url of endpoints){
      try{
        ...
        const reply = data.reply || data.message?.content || data.response || 'Jaspal will call you in 5 mins - WhatsApp 99580 37734';
        typingEl.remove();
    ...
    typingEl.remove();
    const fallback = `Thanks! Jaspal will reply in 2 mins. For instant quote WhatsApp <a href="https://wa.me/${PHONE}?text=${encodeURIComponent(userText)}" target="_blank" style="color:#1e40af;font-weight:700">99580 37734</a> pe message karo.`;
    addMsg(fallback, 'bot');
  }
  ...
  console.log('Jaspal Ollama Chat FIXED loaded - Model:', MODEL);
})();
